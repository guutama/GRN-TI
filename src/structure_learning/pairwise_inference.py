...
def pairwise_inference(in_path,out_path,a_genes_only=True):
    ...
    network_types = ["p0","p2p3","p2p5","p"]
    # Load each DataFrame individually
    exp_a_train = pd.read_csv(os.path.join(in_path, "exp_a_train.csv.gz"))
    exp_all_train = pd.read_csv(os.path.join(in_path, "exp_all_train.csv.gz"))
    eqtl_best_train = pd.read_csv(os.path.join(in_path, "eqtl_best_train.csv.gz"))
...
if __name__ == '__main__':
    in_path =SPLIT_PATH
    out_path = PAIRWISE_PROBABILITY_PATH 
    if not out_path.exists():
        out_path.mkdir(parents=True)
