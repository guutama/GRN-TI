...
def infer(in_path, out_path):
    ...
    # print(f'{len(list(dag_graph.nodes()))=}')
    # print(dag_graph)
    

    # save graph object to file in pickle format
    out_file = os.path.join(out_path, f"dag_graph_fdr20_{network_type}.pickle")
    pickle.dump(dag_graph , open(out_file, 'wb'))

    visualize_path = NETWORKS_INFO_PATH
    visualize_save_file = os.path.join(visualize_path,f"dag_graph_{network_type}.csv")
    ...
    print(f'{FDR=}')
    print(dag_graph)
    graph_info_path = os.path.join(visualize_path,f"graph_info{network_type}.csv")
...
if __name__ == '__main__':
    in_path=PAIRWISE_PROBABILITY_PATH, 
    out_path = NETWORKS_PATH
    if not out_path.exists():
        out_path.mkdir(parents=True)
    infer(in_path=PAIRWISE_PROBABILITY_PATH, 
          out_path = NETWORKS_PATH)